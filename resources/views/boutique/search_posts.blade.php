...
<style type="text/css">
	nav.navbar-webmaster ul.navbar-nav #active7 a{ background:#e0e0e0; border: 2px 0px 2px 0px #779c19; color:#0089ca;}
</style>
...
<div style="width:88%; background-color:#a0a0a0;margin-left:5%; margin-right:5%;margin-bottom:15px; font-size:0.93em;">
						...
						<div>
							...
							<div>
								@if($inte->contains($post->id))
								 ...
								 @else
									<a href="/boutique/interest/{{ $post->id }} "class="btn btn-primary" style="float:right;margin-bottom:15px;">Send Interest</a>
								@endif
							</div><br><br><br>
	...
	@endif
	<div class="row" ><div class="pagination" style="clear:both;float:right;margin-right:35px;"> {{  $Sposts->render() }}</div></div>
